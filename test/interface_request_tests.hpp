...
using namespace roboclaw_serial;
...
protected:
    ...
    void executeTest(const TestData<RequestType>& p)
    {
        ...
        auto device = std::make_shared<DummyTestDevice>(std::move(p.data.readBytes), std::move(p.data.writeBytes));
...
TEST_F(TestExecutor, WriteVelocityPIDConstantsM1SerializationTest)
{
    this->executeTest(TestData<VelocityPIDConstantsM1>
    {
        ...
        {
            "write", // requestType
            {std::byte{0x80}, std::byte{0x1c}, std::byte{0x00}, std::byte{0x00}, std::byte{0x00}, std::byte{0x00}, std::byte{0x00}, std::byte{0x00}, std::byte{0x21}, std::byte{0xb7}, std::byte{0x00}, std::byte{0x00}, std::byte{0x03}, std::byte{0x6e}, std::byte{0x00}, std::byte{0x03}, std::byte{0x13}, std::byte{0x5a}, std::byte{0x7d}, std::byte{0x8e}}, // writeBytes
...
TEST_F(TestExecutor, ReadEncoderCountersSerializationTest)
{
    this->executeTest(TestData<EncoderCounters>
    {
        {12528, 53212}, // request arguments
        {
            ...
            {std::byte{0x00}, std::byte{0x00}, std::byte{0x00}, std::byte{0x00}, std::byte{0x00}, std::byte{0x00}, std::byte{0x30}, std::byte{0xf0}, std::byte{0x00}, std::byte{0x00}, std::byte{0x00}, std::byte{0x00}, std::byte{0x00}, std::byte{0x00}, std::byte{0xcf}, std::byte{0xdc}, std::byte{0xcf}, std::byte{0x2e}} // readBytes
        }
    });
}
